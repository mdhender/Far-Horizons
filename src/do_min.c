...
extern char  input_line[256], mining_done[1000];
...
do_MINING_command() {
    ...
    /* Check if MINING order was already executed for this planet. */
    if (mining_done[nampla_index]) {
        fprintf(log_file, "!!! Order ignored:\n");
        fprintf(log_file, "!!! %s", input_line);
        fprintf(log_file, "!!! Only one MINING command allowed per planet.\n");
        return;
    }
    ...
    old_base = nampla->mi_base;
    if (value == 0) {
        increment = (3 * old_base) / 100;
        if (increment > balance) {
            increment = balance;
        }
        if (increment > nampla->pop_units) {
            increment = nampla->pop_units;
        }
        if (increment == 0) {
            return;
        }
    ...
    nampla->mi_base += increment;
    log_string("    Mining base was increased by ");
    log_long(increment / 10); log_char('.'); log_long(increment % 10);
    log_string(" (from ");
    log_long(old_base / 10); log_char('.'); log_long(old_base % 10);
    log_string(" to ");
    log_long(nampla->mi_base / 10); log_char('.'); log_long(nampla->mi_base % 10);
    log_string(").\n");

    mining_done[nampla_index] = TRUE;
}

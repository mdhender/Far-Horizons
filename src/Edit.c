...
edit_planet() {
    ...
    i = get_value();
    if (i == 1) {
        planet->temperature_class = value;
    }else if (i == -1) {
        return;
    }
    ...
    i = get_value();
    if (i == 1) {
        planet->mining_difficulty = value;
    }else if (i == -1) {
        return;
    }
...
edit_species_stats() {
    ...
    if (answer[0] == 27) {
        ...
        strcpy(species->govt_type, name);
    }

    printf("\nCurrent economic units = %ld: ", species->econ_units);
    i = get_value();
    if (i == 1) {
        species->econ_units = value;
    }else if (i == -1) {
        return;
    }

...
get_gases:
        for (i = first_gas; i <= first_gas + 4; i++) {
            ...
            if (i == HE) {      /* Treat Helium specially. */
                if (rnd(3) > 1) {
                    continue;   /* Don't want too many He planets. */
                }
                if (tc > 5) {
                    continue;             /* Too hot for helium. */
                }
                ++num_gases_found;
...
int LSN(struct planet_data *current_planet, struct planet_data *home_planet) {
    ...
    for (j = 0; j < 4; j++) {   /* Check gases on planet. */
        if (current_planet->gas[j] == 0) {
            continue;
        }

        if (current_planet->gas[j] == O2) {
            ls_needed -= 2;
        }
